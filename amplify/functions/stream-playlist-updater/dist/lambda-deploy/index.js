...
async function downloadAllFilesToEC2(downloads) {
    ...
    const commands = [
        ...
        ...downloads.map(({ s3Url, localPath }) => `echo "Downloading ${localPath}..." && aws s3 cp "${s3Url}" "${localPath}" --region eu-west-1 --quiet && chmod 644 "${localPath}"`),
    ...
    for (let i = 0; i < 24; i++) { // 24 * 5s = 120s = 2 minutes
        ...
        try {
            ...
            if (i < 3)
                continue; // First few checks might fail, keep retrying
            throw err;
        }
    }
